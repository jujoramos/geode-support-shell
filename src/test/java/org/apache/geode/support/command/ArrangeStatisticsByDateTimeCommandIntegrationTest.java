...
import org.apache.geode.support.TreeCopier;
...
public class ArrangeStatisticsByDateTimeCommandIntegrationTest {
  @Autowired
  public Shell shell;
  ...
  public void setUp() throws IOException {
    rootFolder = temporaryFolder.newFolder("otherFiles");
    matchingFolder = temporaryFolder.newFolder("matchingFiles");
    nonMatchingFolder = temporaryFolder.newFolder("nonMatchingFiles");

    TreeCopier treeCopier = new TreeCopier(SampleDataUtils.rootFolder.toPath(), rootFolder.toPath(), true);
    Files.walkFileTree(SampleDataUtils.rootFolder.toPath(), treeCopier);
  }
  ...
  public void springIntegrationTest() throws IOException {
    shell.run(new InputProvider() {
      ...
      public Input readInput() {
        if (!invoked) {
          invoked = true;
          String command = "arrange-statistics-by-date-time"
              + " --sourceFolder " + rootFolder.getAbsolutePath()
  ...
  @Test
  public void arrangeStatisticsByDateTimeShouldGracefullyIntegrateWithSpringShell() {
    MethodTarget methodTarget = shell.listCommands().get("arrange-statistics-by-date-time");
    ...
    assertThat(methodTarget.getHelp()).isEqualTo("Scan the statistics files contained within the source folder and move them to different folders, depending on whether they match the specified date time or not.");
    assertThat(methodTarget.getMethod()).isEqualTo(ReflectionUtils.findMethod(StatisticsCommands.class, "arrangeStatisticsByDateTime", Integer.class, Integer.class, Integer.class, Integer.class, Integer.class, Integer.class, File.class, File.class, File.class, ZoneId.class));
  }

  @Test
  public void arrangeStatisticsByDateTimeShouldThrowExceptionWhenSourceFolderDoesNotExist() {
    String command = "arrange-statistics-by-date-time"
  ...
  @Test
  public void arrangeStatisticsByDateTimeShouldReturnCorrectlyWhenNoFilesAreFound() throws IOException {
  ...
  @Test
  public void arrangeStatisticsByDateTimeShouldUseFileZoneIdForFilteringWhenNoCustomZoneIdIsSpecified() throws IOException {
    String command = "arrange-statistics-by-date-time"
        + " --sourceFolder " + rootFolder.getAbsolutePath()
        + " --matchingFolder " + matchingFolder.getAbsolutePath()
        + " --nonMatchingFolder " + nonMatchingFolder.getAbsolutePath()
    ...
    assertThat(resultsTable.getValue(1, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER1_LOCATOR.getRelativeFilePath());
    assertThat(resultsTable.getValue(1, 1)).isEqualTo("true");
    assertThat(resultsTable.getValue(2, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER1_SERVER1.getRelativeFilePath());
    assertThat(resultsTable.getValue(2, 1)).isEqualTo("true");
    assertThat(resultsTable.getValue(3, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER1_SERVER2.getRelativeFilePath());
    assertThat(resultsTable.getValue(3, 1)).isEqualTo("true");
    assertThat(resultsTable.getValue(4, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER2_LOCATOR.getRelativeFilePath());
    assertThat(resultsTable.getValue(4, 1)).isEqualTo("false");
    assertThat(resultsTable.getValue(5, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER2_SERVER1.getRelativeFilePath());
    assertThat(resultsTable.getValue(5, 1)).isEqualTo("false");
    assertThat(resultsTable.getValue(6, 0)).isEqualTo(SampleDataUtils.SampleType.CLUSTER2_SERVER2.getRelativeFilePath());
    assertThat(resultsTable.getValue(6, 1)).isEqualTo("false");
    assertThat(resultsTable.getValue(7, 0)).isEqualTo(SampleDataUtils.SampleType.CLIENT.getRelativeFilePath());
    ...
    // Assert File Movement.
    assertThat(Files.list(rootFolder.toPath()).filter(path -> Files.isRegularFile(path)).count()).isEqualTo(1);
  ...
  @Test
  public void arrangeStatisticsByDateTimeShouldUseCustomZoneIdForFilteringWhenSpecified() throws IOException {
    String command = "arrange-statistics-by-date-time"
        + " --sourceFolder " + rootFolder.getAbsolutePath()
        + " --matchingFolder " + matchingFolder.getAbsolutePath()
        + " --nonMatchingFolder " + nonMatchingFolder.getAbsolutePath()
